...
And make sure CUDA as version >=11.2. 
"""
import numpy as np
...
def gaussian_kernel_rasterize(coords, mass, center, fov, dims=[0, 1], pixels=512, n_neighbors=64, use_gpu=False, batch_size=1):
    """
...
def distributed_strategy(args):
    ...
    for i in range(THIS_WORKER-1, subhalo_ids.size, N_WORKERS):
        ...
        kappa, alpha, psi, gamma1, gamma2, kappa_variance, alpha_variance = gaussian_kernel_rasterize(
                                            ...
                                            n_neighbors=args.n_neighbors, use_gpu=args.use_gpu,
...
if __name__ == '__main__':
    ...
    parser.add_argument("--offsets",            required=True,              help="Path to offset file (hdf5)")
    ...
    parser.add_argument("--base_filenames",     default="hkappa")
    ...
    parser.add_argument("--fov",                default=1.05,   type=float, help="Field of view of a scene in comoving Mpc")
    ...
    parser.add_argument("--batch_size",         default=32, type=int,       help="Number of particles to rasterize at the same time")
