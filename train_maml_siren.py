...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        for i in range(args.batch_size):
            ...
            axs[i, -2].imshow(model_output["model_out"][i].view(pix, pix).detach().cpu().numpy())
            axs[i, -2].set_title(f"Inner step {args.num_adaptation_steps}")
            axs[i, -2].axis("off")
            axs[i, -1].imshow(targets["image"].view(pix, pix).detach().cpu().numpy())
            axs[i, -1].set_title("Ground Truth")
            axs[i, -1].axis("off")
            for step in range(args.num_adaptation_steps):
                axs[i, step].imshow(model_output["intermed_predictions"][step][i].view(pix, pix).detach().cpu().numpy())
                axs[i, step].axis("off")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--data_len", default=1000, type=int)
    parser.add_argument("--epochs",  default=100, type=int)
    parser.add_argument("--first_omega", default=1/11., type=float)
    parser.add_argument("--hidden_omega", default=1/11., type=float)
    parser.add_argument("--hidden_layers", default=2, type=int, help="Number of SIREN hidden layers")
    parser.add_argument("--hidden_features", default=128, type=int, help="Number of SIREN hidden feature per layers")
    parser.add_argument("--learning_rate", default=1e-2, type=float, help="Outer loop learning rate")
    ...
    parser.add_argument("--num_adaptation_steps", default=3, type=int, help="inner optimization steps")
    parser.add_argument("--batch_size", default=10, type=int, help="Number of tasks in the inner loop")
